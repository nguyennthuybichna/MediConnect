...
export const CLINICAL_DISEASES_KB = [
  {
...
/**
 * Thuật toán suy diễn lâm sàng tính điểm trọng số khớp triệu chứng đa cụm từ
 * trên toàn bộ 24 bệnh lý MediConnect.
 */
export const diagnoseSymptomsClinical = (text) => {
  ...
  for (const item of CLINICAL_DISEASES_KB) {
    ...
    for (const kw of item.keywords) {
      ...
      if (clean.includes(kwClean)) {
        // Multi-word phrase gets higher weight (e.g. "sổ mũi", "đau nửa đầu", "mụn trứng cá")
    ...
    if (score > highestScore) {
      highestScore = score;
      const confidence = Math.min(96.5, Math.round((item.baseConfidence * 100 + Math.min(8, (score - 1) * 1.5)) * 10) / 10);
      bestMatch = {
  ...
  // Nếu không khớp từ khóa đặc hiệu, nhận diện triệu chứng chung
